...
public class ProviderAndRoleElement implements HtmlGeneratorElement, FormSubmissionControllerAction {
	...
	public ProviderAndRoleElement(FormEntryContext context, Map<String, String> parameters) throws BadFormDesignException {
		...
		try {
			...
			//Debut test
			if (tag != null) {
				System.out.println("Contenu du tag :" + tag.toString());
			}
			...
			if (tag.getEncounterRole() == null) {
				//---------------
				System.out.println("Encounter role null");
				...
				if (context.getExistingEncounter() != null) {
					...
					System.out.println("Il existe un existing encounter");
					Map<EncounterRole, Set<Provider>> byRoles = context.getExistingEncounter().getProvidersByRoles();
					if (byRoles.size() > 0) {
						//debut test
						if (byRoles != null) {
							System.out.println("VOICI byRoles" + byRoles.toString());
						}
						...
						//Debut test
						if (p != null) {
							System.out.println("Provider" + p.toString() + "Nom " + p.getName());
						}
			...
			// handle the case where an encounter role is specified
				...
				if (byRole != null) {
					...
					System.out.println("Nous sommes dans le cas ou dans le deuxième cas pour Encounter ");
					System.out.println("byRole :" + byRole.toString());
					if (byRole.get(1) != null) {
						System.out.println("Premier élément Role  :" + byRole.get(1).toString());
					}
				}
				for (int currentIteration = 0; currentIteration < tag.getCount(); currentIteration++) {
					...
					System.out.println("Fait : tWidget providerWidget = tag.instantiateProviderWidget()");
					ErrorWidget providerErrorWidget = new ErrorWidget();
					context.registerWidget(providerWidget);
					System.out.println("Fait : tWidget providerWidget =context.registerWidget(providerWidget);");
					context.registerErrorWidget(providerWidget, providerErrorWidget);
					providerWidgets.add(providerWidget);
					System.out.println("Fait : tWidget providerWidget =providerWidgets.add(providerWidget);");
					providerErrorWidgets.add(providerErrorWidget);
					
					if (byRole != null && byRole.size() > currentIteration) {
						providerWidget.setInitialValue(byRole.get(currentIteration));
						
						System.out.println("on est ici  : providerWidget.setInitialValue(byRole.get(currentIteration));");
						initialProviderSet = true;
					}
				}
			}
			...
			if (!initialProviderSet && providerWidgets.iterator().next() != null) {
				...
				//Test
				System.out.println("On est ici : if (!initialProviderSet && providerWidgets.iterator().next() != null)");
				Provider provider = tag.getDefaultValue();
				if (provider != null) {
					providerWidgets.iterator().next().setInitialValue(provider);
					
					System.out.println("on est ici : providerWidgets.iterator().next().setInitialValue(provider);");
				}
			}
		}
		catch (Exception e) {
